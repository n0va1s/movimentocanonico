@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}">
        ...
        <div class="flex items-center justify-center gap-1.5 flex-wrap sm:hidden w-full">
            {{-- Previous Page Link --}}
            @if ($paginator->onFirstPage())
                <span class="inline-flex items-center px-3 py-2 text-xs sm:text-sm font-medium text-gray-400 bg-white border border-gray-200 cursor-not-allowed leading-5 rounded-md dark:text-zinc-500 dark:bg-zinc-800 dark:border-zinc-700">
                    Anterior
                </span>
            @else
                <a href="{{ $paginator->previousPageUrl() }}" rel="prev" class="inline-flex items-center px-3 py-2 text-xs sm:text-sm font-medium text-zinc-700 bg-white border border-zinc-300 leading-5 rounded-md hover:text-zinc-500 focus:outline-none focus:ring-2 focus:ring-blue-500 active:bg-zinc-100 transition ease-in-out duration-150 dark:bg-zinc-800 dark:border-zinc-700 dark:text-zinc-300 dark:hover:bg-zinc-700">
                    Anterior
                </a>
            @endif
            ...
            @for ($page = $startPage; $page <= $endPage; $page++)
                @if ($page == $paginator->currentPage())
                    <span aria-current="page">
                        <span class="inline-flex items-center px-3.5 py-2 text-xs sm:text-sm font-bold text-white bg-blue-600 border border-blue-600 leading-5 rounded-md dark:bg-blue-500 dark:border-blue-500 shadow-sm">
                            {{ $page }}
                        </span>
                    </span>
                @else
                    <a href="{{ $paginator->url($page) }}" class="inline-flex items-center px-3.5 py-2 text-xs sm:text-sm font-medium text-zinc-700 bg-white border border-zinc-300 leading-5 rounded-md hover:text-zinc-500 focus:outline-none focus:ring-2 focus:ring-blue-500 active:bg-zinc-100 transition ease-in-out duration-150 dark:bg-zinc-800 dark:border-zinc-700 dark:text-zinc-300 dark:hover:bg-zinc-700">
                        {{ $page }}
                    </a>
                @endif
            @endfor
            ...
            @if ($paginator->hasMorePages())
                <a href="{{ $paginator->nextPageUrl() }}" rel="next" class="inline-flex items-center px-3 py-2 text-xs sm:text-sm font-medium text-zinc-700 bg-white border border-zinc-300 leading-5 rounded-md hover:text-zinc-500 focus:outline-none focus:ring-2 focus:ring-blue-500 active:bg-zinc-100 transition ease-in-out duration-150 dark:bg-zinc-800 dark:border-zinc-700 dark:text-zinc-300 dark:hover:bg-zinc-700">
                    Próximo
                </a>
            @else
                <span class="inline-flex items-center px-3 py-2 text-xs sm:text-sm font-medium text-gray-400 bg-white border border-gray-200 cursor-not-allowed leading-5 rounded-md dark:text-zinc-500 dark:bg-zinc-800 dark:border-zinc-700">
                    ...
                    @foreach ($elements as $element)
                        ...
                        @if (is_array($element))
                            @foreach ($element as $page => $url)
                                @if ($page == $paginator->currentPage())
                                    <span aria-current="page">
                                        <span class="inline-flex items-center px-4 py-2 -ml-px text-sm font-semibold text-white bg-blue-600 border border-blue-600 cursor-default leading-5 dark:bg-blue-500 dark:border-blue-500 shadow-sm">{{ $page }}</span>
                                    </span>
                                @else
