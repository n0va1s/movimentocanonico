...
new class extends Component {
    ...
    {
        $this->mensagem = $mensagem->load(['evento.movimento', 'usuario']);
    }
    ...
    public function marcarComoEnviado(int $envioId): void
    {
        ...
        $this->dispatch('notify', message: "Status de {$envio->nom_destinatario} atualizado!");
    }
    ...
    public function resetarEnvio(int $envioId): void
    {
        ...
        $this->dispatch('notify', message: "Envio de {$envio->nom_destinatario} resetado para pendente.");
    }
    ...
    public function dispararProximoPendente(): void
    {
        ...
        if (!$proximo) {
            $this->dispatch('notify', message: 'Parabéns! Todos os contatos desta campanha já foram enviados.', variant: 'success');
            return;
        }
        ...
        // Dispara o evento de abertura no navegador
        $this->dispatch('abrir-whatsapp', url: $url);
        $this->dispatch('notify', message: "Abrindo chat de {$proximo->nom_destinatario}...");
    }
    ...
    public function gerarUrlWhatsapp(MensagemEnvio $envio): string
    {
        $data = [
            ...
            'evento' => $this->mensagem->evento->des_evento,
            'participante' => $envio->nom_responsavel ?: '',
            'responsavel_nome' => $envio->nom_responsavel ? $envio->nom_destinatario : '',
        ];

        // Formata dinamicamente aplicando placeholders e Spintax
        $textoFormatado = Mensagem::formatar($this->mensagem->txt_mensagem, $data);

        // Limpa e prepara o número do telefone com DDI 55
            ...
            ->when($this->search, function ($query) {
                $query->where(function ($q) {
                    $q->where('nom_destinatario', 'like', '%' . $this->search . '%')
                        ->orWhere('nom_responsavel', 'like', '%' . $this->search . '%')
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <flux:card class="xl:col-span-2 space-y-4 border border-zinc-200 dark:border-zinc-700 shadow-sm rounded-xl">
            <div>
                ...
                <flux:subheading class="mt-1">
                    Evento: <strong class="text-zinc-800 dark:text-zinc-200">{{ $mensagem->evento->des_evento }}</strong>
                </flux:subheading>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-x-6 gap-y-2 text-xs text-zinc-500">
                ...
                <div>
                    Criado por: <strong class="text-zinc-700 dark:text-zinc-300">{{ $mensagem->usuario->name }}</strong>
                </div>
                <div>
                    Criado em: <strong class="text-zinc-700 dark:text-zinc-300">{{ $mensagem->created_at->format('d/m/Y H:i') }}</strong>
                </div>
            </div>
        </flux:card>

        {{-- Progresso & Ação Rápida --}}
        <flux:card class="space-y-6 flex flex-col justify-between border border-zinc-200 dark:border-zinc-700 shadow-sm rounded-xl">
            ...
            @if ($sucesso_count < $total_count)
                <flux:button
                    wire:click="dispararProximoPendente"
                    variant="primary"
                    icon="paper-airplane"
        ...
        <flux:card class="overflow-x-auto p-0 border border-zinc-200 dark:border-zinc-700 shadow-sm rounded-xl">
            <flux:table>
                <flux:table.columns>
                    ...
                    @if ($mensagem->tip_destinatario === 'R')
                        <flux:table.column>Participante Associado</flux:table.column>
                    @endif
                ...
                <flux:table.rows>
                    @forelse ($envios as $envio)
                        <flux:table.row :key="'envio-'.$envio->idt_mensagem_envio">
                            ...
                            @if ($mensagem->tip_destinatario === 'R')
                                <flux:table.cell class="text-sm text-zinc-800 dark:text-zinc-200">
                                    {{ $envio->nom_responsavel ?: '—' }}
                                </flux:table.cell>
                            @endif
